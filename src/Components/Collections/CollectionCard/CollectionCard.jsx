...
const CollectionCard = ({ product, handleAddToCart }) => {
  ...
  useEffect(() => {
    const handleVariant = () => {
      setVariant({
        [product.variant_groups[0].id]: color,
        [product.variant_groups[1].id]: size,
        [product.variant_groups[2].id]: gender,
      });
    };
    const handleImgs = () => {
      let imgbx = [];
      product.assets.map((img) => imgbx.push(img.url));
        ...
        <div className="imgCircles">
          {productImgs.map((nums, index) => (
            <div
              key={index}
              className={`circle ${
                index === Imgindex ? "Imgaction" : "circle"
              }`}
            ></div>
          ))}
        </div>
          ...
          <div className="size">
            ...
            <div className="sizelist">
              {product.variant_groups[1].options.map((sizebtn, index) => (
                <button
                  key={sizebtn.id}
                  className={`transparet blackBorder sizeBtn ${
                    s === sizebtn.name ? "activeBtn" : "blackBorder"
                  }`}
            ...
            <div className="colorlist">
              {product.variant_groups[2].options.map((genderbtn, index) => (
                <button
                  key={genderbtn.id}
                  className={`transparet blackBorder sizeBtn ${
                    g === genderbtn.name ? "activeBtn" : "blackBorder"
                  }`}
                  onClick={() => getGender(index)}
                >
                  {genderbtn.name}
                </button>
              ))}
